...
const SignInSignUpPage = () => {
  ...
  const handleSubmitLogIn = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        localStorage.setItem('usercnic', decoded.username);
        login();
        setError("");
        navigate("/");
      } else {
  ...
  const handleSubmitSignUp = async (e) => {
    ...
    if (!/^[A-Za-z\s]{3,255}$/.test(formData.name)) {
      validationErrors.name =
        "Name must contain only alphabets and spaces, and be between 3 and 255 characters long.";
    }

    // CNIC validation
    if (!/^\d{13}$/.test(formData.cnic)) {
      validationErrors.cnic =
        "Should be 13 digits long and contain only numbers.";
    }
  ...
  return (
    <>
      ...
      >
        <div
          className="tab-content col-md-4 shadow-1-strong"
